...
export function TokenSelectModal({
  ...
  const filteredTokens = tokens.filter(token => {
    ...
    // Filter by network
    const matchesNetwork = selectedNetwork === "All Networks" || token.network === selectedNetwork;
        ...
        <div className="flex items-center p-5 border-b border-[#3a4155]">
          ...
          >
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
              <path d="M19 12H5M5 12l7-7m-7 7l7 7" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
          <h3 className="text-xl text-white font-medium flex-1 text-center">Select A Token</h3>
        </div>
        ...
        <div className="p-4 flex gap-4">
          ...
          <div className="relative">
            ...
            >
              ...
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" className={`transition-transform ${showNetworkDropdown ? 'rotate-180' : ''}`}>
            ...
            {showNetworkDropdown && (
              <div className="absolute right-0 mt-2 bg-[#1f2533] border border-[#3a4155] rounded-xl w-full z-10 shadow-lg">
                {networks.map((network) => (
                  <button
                    key={network}
                    className={`w-full text-left px-4 py-2 text-white hover:bg-[#3a4155] ${selectedNetwork === network ? 'bg-[#3a4155]' : ''}`}
                    onClick={() => {
            ...
            filteredTokens.map((token, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <Image src={token.logo} alt={token.symbol} width={40} height={40} className="rounded-full" />
